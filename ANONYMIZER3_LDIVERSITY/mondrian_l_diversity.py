...
class Partition(object):
    ...
    def __init__(self, data, width, middle):
        print "[LOG] Initializing with data, width and middle"
        self.member = data[:]
    ...
    def __len__(self):
        # print "[LOG] Returning the number of records in partition"
        return len(self.member)

def check_L_diversity(partition):
    print"[LOG] Checking if partition satisfy l-diversity."
    sa_dict = {}
...
def get_normalized_width(partition, index):
    # print "[LOG] Returning normalized width of partition similar to NCP"
...
def choose_dimension(partition):
    print "[LOG] Choosing dimension with largest normalized Width return dim index."
    max_witdh = -1
    max_dim = -1
...
def frequency_set(partition, dim):
    print "[LOG] Getting the frequency_set of partition"
    frequency = {}
...
def find_median(partition, dim):
    print "[LOG] Finding the middle of the partition"
...
def split_numerical_value(numeric_value, splitVal):
    print "[LOG] Splitting numeric value on splitVal"
...
def split_numerical(partition, dim, pwidth, pmiddle):
    print "[LOG] Strict splitting numeric attribute by finding a median, lhs = [low, means], rhs = (mean, high]"
    sub_partitions = []
    # numeric attributes
...
def split_categorical(partition, dim, pwidth, pmiddle):
    print "[LOG] Splitting categorical attribute using generalization hierarchy"
    sub_partitions = []
...
def split_partition(partition, dim):
    print "[LOG] Splitting partition and distributing records to different sub-partitions"
...
def anonymize(partition):
    print "[LOG] Main procedure of Half_Partition. Recursively partition groups until not allowable."
...
def check_splitable(partition):
    print "[LOG] Checking if the partition can be further splitted while satisfying k-anonymity."
...
def init(att_trees, data, L):
    print "[LOG] Resetting global variables"
