...
def read_pickle_file(att_name):
    ...
    """
    try:
        static_file = open('data/informs_dataset/informs_' + att_name + '_static.pickle', 'rb')
...
def read_tree_file(treename):
    ...
    leaf_to_path = {}
    att_tree = {}
    prefix = 'data/informs_dataset/informs_'
    postfix = ".txt"
...
def read_data(flag=0):
    ...
    """
    data = []
    userfile = open('data/informs_dataset/demographics.csv', 'rU')
    conditionfile = open('data/informs_dataset/conditions.csv', 'rU')
    userdata = {}
    numeric_dict = []
    QI_num = len(QI_INDEX)
    ...
    for i in range(QI_num):
        if IS_CAT[i] is False:
            static_file = open('data/informs_dataset/informs_' + USER_ATT[QI_INDEX[i]] + '_static.pickle', 'wb')
